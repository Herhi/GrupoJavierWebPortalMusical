...
function insertarInvoiceLine($InvoiceId, $cesta){
    ...
    foreach ($cesta as $id) {
        ...
        $stmt = $conexion->prepare("SELECT UnitPrice FROM Track WHERE TrackId='$id'");
        $stmt->execute();
        ...
        foreach($stmt->fetchAll() as $row) {
            ...
            if(in_array($id, $tracksAgregadas)) {


                $stmt2 = $conexion->prepare("UPDATE InvoiceLine SET Quantity=(Quantity+1) WHERE TrackId='$TrackId' AND InvoiceLineId='$InvoiceLineId'");
                ...
                $stmt2 = $conexion->prepare("INSERT INTO InvoiceLine (InvoiceLineId, InvoiceId, TrackId, UnitPrice, Quantity) VALUES (:InvoiceLineId, :InvoiceId, :TrackId, :UnitPrice, :Quantity)");
                    ...
                    array_push($tracksAgregadas, $id);

                    var_dump($tracksAgregadas);
            }
    }
