        ...
        if (!isset($_POST) || empty($_POST)) {
            ...
            if(isset($_POST['agregar'])){
                ...
                if (!empty($_COOKIE['cesta'])){
                    ...
                    $cesta=unserialize($_COOKIE['cesta']);
                    //Creamos un array asociativo auxiliar de la $cesta
                    $cestaAux=array();
                    foreach ($cesta as $key => $cancion) {
                    	if (array_key_exists($cancion, $cestaAux)) {
                    		$cestaAux[$cancion]+=1;
                    	}else{	
                    		$cestaAux[$cancion]=1;
                    	}       	
                    ...
                    imprimirCesta($cestaAux);

                    $InvoiceId=altaPedido(($_COOKIE['userId']), $total);
                    insertarInvoiceLine($InvoiceId, $cestaAux);
                    
                    //Aqui habria que vaciar la cesta
                     $cesta=array();
                		setcookie('cesta', serialize($cesta), time() - (86400 * 30), "/");
