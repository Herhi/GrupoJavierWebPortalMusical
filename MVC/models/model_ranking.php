...
function obtenerRanking($customerId) {
	...
	try {
		...
		$consulta = $conexion->prepare("SELECT count(*) as Descargas, t.* FROM Track t, InvoiceLine i, Invoice WHERE t.TrackId=i.TrackId and Invoice.InvoiceId=i.InvoiceId and Invoice.CustomerId=$customerId group by t.name order by t.name");
